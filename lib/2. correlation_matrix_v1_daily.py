...
collection_correlation_matrix = 'correlation_matrix_daily'
time_query = {}
projection = {'timestamp': 1}
fc = 0.0  # this allows to save only correlations that are bigger than fc
# -------------------------------------------------
...
def get_list_subcategory(sub_category, field):
    r_query = {}
    # Subcategory rules for the correlation calculation
    # $V_1$ for the North East part of the building
    if sub_category == 'NE':
        r_query[field] = {'$in': ['NE', 'N', 'E', 'N/A']}
        return r_query
    # $V_2$ for the South West part of the building
    if sub_category == 'SW':
...
def compute_matrix_correlation(tag_query):
    ...
    time_line = pd.date_range(start_time, end_time + offset, freq='1D')
