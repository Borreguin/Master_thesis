...
selected_category = 'B'
collection_source = "vector_daily"
...
label_name = label_category[selected_category]
selected_vector = vector_names[selected_category]

# HMM model Parameters
hmm_path_model = '../HMM_models/Final_models/'
file_names = {
    'A': ['named_hmm_R_vector_A_A-A_3-A_4_1-A_4_2-A_6_1-A_6_2_10.pkl'],
    'B': ['named_hmm_R_vector_B_A-A_3-A_4_1-A_4_2-A_6_1-A_6_2_10.pkl'],
    'C': ['named_hmm_S_vector_C_C_1-C_2-C_4_5.pkl',
...
def save_results(df_result, hidden_state_labels):
    ...
    df_to_save['hidden_state_label'] = df_result[file_names[selected_category][-1]]

    N = len(df_to_save.index)
    counter2 = 0
    total_reg = 0
    for idx in list(df_to_save.index):
        name_state_list = list(df_result.loc[idx, file_names[selected_category]])
    ...
    print("\nThe models \n{0} \nhave a coincidence of {1}\n".format(file_names[selected_category], counter2 / N * 100))
...
def main():
    ...
    df_result = get_dates()
    list_files = file_names[selected_category]
