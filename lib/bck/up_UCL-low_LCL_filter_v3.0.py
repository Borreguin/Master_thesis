...
def main():
    ...
    for tag in tags:
        ...
        global_mean = (df[tag].quantile(q=0.25)+ df[tag].quantile(q=0.5) + df[tag].quantile(q=0.75))/3
        global_std = ((df[tag].quantile(q=0.25)-global_mean)**2 + (df[tag].quantile(q=0.75)-global_mean)**2)**0.5
        ...
        delta_6d = pd.DateOffset(days=6)
        delta_14d = pd.DateOffset(days=14)
        ...
        for t in periods:
            t_fin = t + delta_6d + delta_24h
            # inspected day
            df_raw = df.loc[t:t_fin].copy()
            # getting statistics
            df_past = df.loc[t - delta_14d:t]
            df_future = df.loc[t:t_fin + delta_14d]

            if df_past.size > 5 and df_future.size > 5:
                u_mean = (df_raw[tag].mean() + df_past[tag].mean() + df_future[tag].mean() + global_mean) /4
                u_std = (df_raw[tag].std() + df_past[tag].std() + df_future[tag].std() + global_std) / 4
            elif df_past.size > 5:
                u_mean = (df_raw[tag].mean() + df_past[tag].mean() + global_mean) / 3
                u_std = (df_raw[tag].std() + df_past[tag].std() + global_std) / 3
            elif df_future.size > 5:
                u_mean = (df_raw[tag].mean() + df_future[tag].mean()+ global_mean) / 3
                u_std = (df_raw[tag].std() + df_future[tag].std() + global_std) / 3
            else:
            ...
            for idx in range(number_interactions):
                ...
                cls_values = df_raw[(df_raw[tag] > lcl) & (df_raw[tag] < ucl)]
                if not cls_values.empty:
                    u_mean = (cls_values[tag].mean()+ global_mean)/2
                    u_std = (cls_values[tag].std() + global_std)/2
