...
class SAXModel(object):
    def __init__(self, window=None, stride=None,
                 nbins=None, alphabet=None, cutpoints=None):
        """
        ...
        if cutpoints is None:
            if not (2 <= self.nlevels <= 10):
                raise ValueError("alphabet size is within 2 and 10 for current impl.")
            self.cutpoints = {2: [-np.inf, 0.5, np.inf],
            ...
            if isinstance(cutpoints, list):
                self.cutpoints = {len(self.alphabet) : cutpoints}
                #print(self.cutpoints)
            else:
                print('cutpoints is a list: Ex: for alphabet=4: [-np.inf, -0.67, 0, 0.67, np.inf]')
        cps = self.cutpoints[len(self.alphabet)]
        # vecs = map(lambda (a, b): (a+b)/2, zip(cps, cps[1:])) ## taking mean may not be accurate
        vecs = [(x + y) / 2 for x, y in zip(cps, cps[1:])]  ## taking mean may not be accurate
        vecs[0] = cps[1]
        vecs[-1] = cps[-2]
    ...
    def symbolize(self, xs):
        ...
        # print(xs)
        # print(cutpoints)
        # print(pd.cut(xs, bins=cutpoints, labels=self.alphabet))
    ...
    def paa_window(self, window_signal):
		...
		"""
        s = window_signal  # when normalization is not needed
        # s = self.whiten(window_signal) #if we need to do normalization
        # print(s)
        ...
        # print("sliding window signal:", xs)
        return xs
    ...
    def symbolize_window(self, window_signal):
		...
		"""
        # s = self.whiten(window_signal) #this allows the normalization per window
        # binsize = Fraction(len(s), self.nbins)
        # xs = map(lambda ss: np.sum(ss) / float(binsize), self.binpack(s))
        xs = self.paa_window(window_signal)
        return "".join(self.symbolize(xs))

    def symbolize_whiten_window(self, window_signal):
        """
        Symbolize one sliding window signal to a word
        """
...
def chunk(xs, chunk_size):
    p = 0
    # print(p,len(xs), chunk_size)
    while p < len(xs):
        ind = int(np.ceil(p + chunk_size))
        yield xs[int(p): int(ind)]
        p += chunk_size
