/**
 * Created by Roberto on 11/15/2016.
 * Adapted from:
 * Copyright (c) 2012, Kai Chang
 * Released under the BSD License: http://opensource.org/licenses/BSD-3-Clause
 */

/* This visualization needs a data set with:
*   name: for each record (this allows th search process)
*   group: for make the different colors and therefore the colors
*   This example runs with d3.V2
* */
...
end_time =   getQueryVariable('end_time');
tag_name = getQueryVariable('tag_name');

if(tag_name == undefined){
    tag_name = 'V022_vent02_CO2';
}

if(start_time == null){
    start_time = '2013-01-10';
}
if(end_time == null){
    end_time = '2013-12-21';
}
...
var m = [50, 0, 10, 0],  //up, right, down, left (only positive values)
    w = width - m[1] - m[3],
    h = height - m[0] - m[2],
    ...
    dragging = {},
    line = d3.svg.line(),
    axis = d3.svg.axis().orient("left").ticks(4+height/25),
...
//Let's define the colors for each group
// hsla colors
var colors = {
    "Non occupied period 1": [325,50,39],
    "Arriving period": [28,100,52],
    "First working period": [20,49,49],
    "Second working period": [359,69,49],
    "Leaving period": [120,56,40],
    "Non occupied period 2": [255,58,73],
    "UCL": [185,56,73],
    "LCL": [185,56,73]
};
...
var list_projection = tag_name;

var data_url = '/data_hmm'+ "?list_projection=" + list_projection + "&start_time=" + start_time
            + "&end_time=" + end_time ;


// Load the data and visualization
// (this definition changes because of d3.json definition of d3.V2)
queue()
    .defer(d3.json, data_url)
    .await(make_coord);
function make_coord(error,raw_data) {
    console.log(error);
    // Convert quantitative scales to floats
    data = raw_data[tag_name].map(function(d) {
    for (var k in d) {
		//&& k != 'id'
      if (!_.isNaN(raw_data[tag_name][0][k] - 0) ) {
...
function highlight(d) {
  ...
  d3.selectAll(".rowh").style("opacity", function(p) { return (d.group == p) ? null : "0.3" });
...
function unhighlight() {
  d3.select("#foreground").style("opacity", null);
  d3.selectAll(".rowh").style("opacity", null);
